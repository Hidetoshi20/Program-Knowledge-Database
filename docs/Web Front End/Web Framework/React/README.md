# React 学习笔记

这是一个关于 React 的学习笔记集合，涵盖了从基础概念到高级特性的各个方面。

## 核心概念

-   [React 特性](./Intro-and-Features.md)
-   [虚拟 DOM (Virtual DOM)](./Virtual%20Dom.md)
-   [组件 (Component)](./component.md)
-   [生命周期 (Lifecycle)](./Life%20circle.md)
-   [事件处理 (Event Handling)](./Event-Handling.md)
-   [Hooks](。/hooks.md)

## 高级主题

-   [路由 (Router)](./Router.md)
-   [组件通信与状态管理 (Redux)](./Contact.md)
-   [性能优化 (Performance)](./performence.md)
-   [错误处理 (Error Handling)](./Error-Handling.md)
-   [动画 (Animation)](./Animation.md)

## 其他

-   [React vs Vue 对比](.。/Comparisons.md)
-   [相关资源](./resource.md)
